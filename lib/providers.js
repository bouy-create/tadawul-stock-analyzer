...
function normalizeCandidates(symbol) {
  ...
  if (/^\d+$/.test(value)) return [value, `${value}.SR`, `${value}.SA`, `${value}:Tadawul`];
...
async function fetchFromYahooChart(symbol, fetchImpl = fetch) {
  ...
  const out = { ok: response.ok && isValidPositivePrice(mapped.currentPrice), source: "yahoo", raw: response.body, mapped, status: response.status };
  setCache(cacheKey, out);
  return out;
}
