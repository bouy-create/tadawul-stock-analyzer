...
async function fetchFromYahooQuoteSummary(symbol) {
  ...
  const classifyYahooError = (error) => {
    const status = Number(error?.statusCode || error?.status || error?.response?.status || 0);
    const message = error?.message || "Yahoo quoteSummary failed";
    return { status, message };
  };

  const requestWithHeaders = async (headers) => yahooFinance.quoteSummary(symbol, {
    modules,
    ...(headers ? { headers } : {})
  });
  ...
  try {
    let summary;
    try {
      summary = await requestWithHeaders();
    } catch (error) {
      const firstError = classifyYahooError(error);
      if (firstError.status !== 401) throw error;

      try {
        // Retry once: yahoo-finance2 should handle crumb/cookie refresh on repeated attempt.
        summary = await requestWithHeaders();
      } catch (retryError) {
        const secondError = classifyYahooError(retryError);
        if (secondError.status !== 401) throw retryError;
        summary = await requestWithHeaders({ "User-Agent": "Mozilla/5.0 (compatible; TadawulStockAnalyzer/1.0)" });
      }
    }
    ...
    const normalized = classifyYahooError(error);
    const out = {
      ok: false,
      source: "yahoo",
      raw: { error: normalized.message, status: normalized.status || 0 },
      mapped: mapYahooQuoteSummary({}, symbol),
...
async function fetchFromFinnhubMetrics(symbol, fetchImpl = fetch) {
  if (!process.env.FINNHUB_KEY) {
    return { ok: false, source: "finnhub-metrics", raw: null, mapped: mapFinnhub({}, {}, {}, symbol), status: 0, skipped: true };
  }

  const cacheKey = `provider:finnhub-metrics:${symbol}`;
  const cached = getCache(cacheKey);
  if (cached) return cached;
  ...
  try {
    ...
    const mapped = mapFinnhub({}, profileResp.body || {}, metricResp.body?.metric || {}, symbol);
    const out = {
      ok: profileResp.ok || metricResp.ok,
      source: "finnhub-metrics",
      raw: { profile: profileResp.body, metric: metricResp.body },
      mapped,
      status: metricResp.status || profileResp.status
    };
    setCache(cacheKey, out);
    return out;
  } catch (error) {
    const out = { ok: false, source: "finnhub-metrics", raw: { error: error.message }, mapped: mapFinnhub({}, {}, {}, symbol), status: 0 };
    setCache(cacheKey, out);
    return out;
  }
}
