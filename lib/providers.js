...
function mapNews(items = []) {
  return items.slice(0, 5).map((item) => ({
    ...
    source: item.source ?? item.creator ?? "rss",
    url: item.link ?? null,
...
async function fetchNews(symbol) {
  const cacheKey = `provider:news:${symbol}`;
  const cached = getCache(cacheKey);
  if (cached) return cached;

  const yahooUrl = `https://finance.yahoo.com/rss/headline?s=${encodeURIComponent(symbol)}`;
  const googleUrl = `https://news.google.com/rss/search?q=${encodeURIComponent(symbol)}`;

  try {
    const yahooFeed = await rssParser.parseURL(yahooUrl);
    const mappedYahoo = mapNews(yahooFeed?.items || []);
    if (mappedYahoo.length > 0) {
      setCache(cacheKey, mappedYahoo);
      return mappedYahoo;
    }
  } catch (error) {
    console.log("[stock-api][news]", { symbol, provider: "yahoo-rss", status: 0, error: error.message });
  }

  try {
    const googleFeed = await rssParser.parseURL(googleUrl);
    const mappedGoogle = mapNews(googleFeed?.items || []);
    setCache(cacheKey, mappedGoogle);
    return mappedGoogle;
  } catch (error) {
    console.log("[stock-api][news]", { symbol, provider: "google-rss", status: 0, error: error.message });
    return [];
  }
}
